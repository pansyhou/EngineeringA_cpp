...
osThreadId Task_BoardCommuHandle;
osThreadId Task_DetectHandle;
...
extern void BoardCommuni_Task(void const * argument);
...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(Task_Remote, Remote_Task, osPriorityHigh, 0, 200);
  Task_RemoteHandle = osThreadCreate(osThread(Task_Remote), NULL);

  /* definition and creation of Task_Gimbal */
  osThreadDef(Task_Gimbal, Gimbal_Task, osPriorityHigh, 0, 200);
  Task_GimbalHandle = osThreadCreate(osThread(Task_Gimbal), NULL);

  /* definition and creation of Task_BoardCommu */
  osThreadDef(Task_BoardCommu, BoardCommuni_Task, osPriorityNormal, 0, 200);
  Task_BoardCommuHandle = osThreadCreate(osThread(Task_BoardCommu), NULL);

  /* definition and creation of Task_Detect */
  osThreadDef(Task_Detect, Detect_Task, osPriorityIdle, 0, 128);
  Task_DetectHandle = osThreadCreate(osThread(Task_Detect), NULL);
